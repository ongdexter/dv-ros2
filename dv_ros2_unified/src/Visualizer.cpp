#include "dv_ros2_unified/Visualizer.hpp"

namespace dv_ros2_unified
{
    Visualizer::Visualizer(const rclcpp::NodeOptions &options)
        : Node("visualizer_component", options), m_node{this}
    {
        RCLCPP_INFO(m_node->get_logger(), "[Visualizer::Visualizer] Initializing...");
        auto node_name = m_node->get_name();
        this->visualizer_ctor(node_name);
        RCLCPP_INFO(m_node->get_logger(), "[Visualizer::Visualizer] Initialization complete!");

        RCLCPP_INFO(m_node->get_logger(), "[Visualizer::Visualizer] Beginning spin...");
        this->start();
    }

    void Visualizer::visualizer_ctor(const std::string &t_node_name)
    {
...
RCLCPP_COMPONENTS_REGISTER_NODE(dv_ros2_unified::Visualizer)