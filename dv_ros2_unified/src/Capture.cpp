...
namespace dv_ros2_unified
{
    ...
    Capture::Capture(const rclcpp::NodeOptions &options)
        : Node("live_capture_component", options), m_node{this}
    {
        RCLCPP_INFO(m_node->get_logger(), "[Capture::Capture] Initializing...");
        auto node_name = m_node->get_name();
        this->capture_ctor(node_name);
        RCLCPP_INFO(m_node->get_logger(), "[Capture::Capture] Initialization complete!");

        RCLCPP_INFO(m_node->get_logger(), "[Capture::Capture] Beginning spin...");
        this->startCapture();
    }

    void Capture::capture_ctor(std::string node_name)
    {
        m_spin_thread = true;
...
RCLCPP_COMPONENTS_REGISTER_NODE(dv_ros2_unified::Capture)