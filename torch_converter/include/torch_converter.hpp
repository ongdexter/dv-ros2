...
namespace torch_converter
{
    struct Params
    {
        ...
        float known_flying_height = 0.1f; // known flying height in meters for depth scaling
    };
    ...
    private:
        ...
        void publish_depth_raw(torch::Tensor &depth_tensor);

        void publish_depth_image(torch::Tensor &depth_tensor);

        void publish_point_cloud(torch::Tensor &point_cloud_tensor);
        ...
        float m_last_known_height = 0.0f;
        ...
        rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr m_ptcloud_publisher;
